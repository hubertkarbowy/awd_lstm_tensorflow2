...
class WeightDropLSTMCell(tf.keras.layers.LSTMCell):
    ...
    if self.implementation == 1:
      ...
      if training is True:
        ...
        if self.modified is False: # works only in eager mode
          ...
          self.modified.assign(True)
        z += K.dot(h_tm1, self.awd_recurrent_kernel)
        #self.modified.assign(True) # has effect only in eager mode
      else:
        z += K.dot(h_tm1, self.recurrent_kernel)
      if self.use_bias:
  ...
  def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
    ...
    self.modified.assign(False)
    self.recurrent_kernel.assign(self.awd_recurrent_kernel)
